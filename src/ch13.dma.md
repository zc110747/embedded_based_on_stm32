...
DMA最重要的应用是用于替代MCU对外设的访问，如前面的usart，dac，adc都支持DMA操作的方法。对于大部分开发者，都是直接从网上移植或者直接使用使用CubeMX生成的代码，并不理解背后的工作逻辑。这不算问题，基于STM32 HAL库的封装使用可以大幅度降低难度。不过对底层工作理解的缺失，如果代码正常工作还好，如果工作不正常，在HAL库的框架去查找问题反而更加复杂。所以在学习中还是需要理解DMA的工作机制，才能更好的使用。
...
以STM32F429的DMA为例，其控制主体是DMAx_Streamx，也就是使用不同控制流的DMA应用可以同时使能，然后按照通道的优先级和编号由DMA模块自动控制先后工作顺序。在DMA的操作中，如果有外设的参与，则需要选择在芯片设计时已经有的固定通道。而对于没有外设参与的memory间传输，则没有限制，可以使用支持数据传输的任意通道。
...
1. 内存到外设，主要用于输出，如Timer_DMA控制PWM输出，DAC_Timer_DMA控制正弦波输出，USART/CAN/SPI/I2C等接口的DMA串口输出
