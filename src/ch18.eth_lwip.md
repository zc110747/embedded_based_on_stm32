...
![image](./image/18_01.png)

- 网络应答，服务端根据请求返回给客户端数据，具体流程如下所示。

![image](./image/18_02.png)
...
![image](./image/18_13.png)

通过ping命令测试网络是否正常，具体如下所示。

![image](./image/18_14.png)
...
至此，关于STM32上的LWIP应用进行了说明。随着STM32CubeMX和其它芯片代码生成工具的发展，在单片机实现网络应用已经越来越简单。不过没有深入去理解PHY、MAC和LWIP、以及TCP/IP协议原理，当开发中如果遇到网络无法连接、通讯丢包或者应用层断开等问题，解决时再去理解由SDK直接生成的代码框架，其实难度反而增大了。这并不是说不该用SDK，既然能大幅度提高效率，使用SDK反而是最优选择。不过在学习单片机网络应用过程中，还是要花时间去理解执行的流程，不能生成完代码，ping通了就认为学习掌握了网络知识，这是远远不足的。如果时间充足，还是单步调试下，配合代码理解LWIP的执行流程，另外LWIP的代码也是十分经典的C语言开源代码，包含很多C语言协议解析技巧，十分值得去了解学习。

## next_chapter

[返回目录](./../README.md)

直接开始下一小节: [低功耗应用说明](./ch19.fmc_sdram.md)
