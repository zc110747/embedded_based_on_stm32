...
USART(通用同步异步收发器)是可以灵活的与外部设备进行数据交互的，支持全双工和半双工，同步和异步收发的通讯模块。在嵌入式系统中，USART可以说是最常用的模块之一，例如用于打印调试信息和命令控制的调试口，用于和蓝牙，wifi模块连接的通讯口，板级芯片通讯的连接口，都是基于USART功能实现。另外USART也支持外扩芯片支持不同的物理接口类似RS232，RS485和RS422这类，可以满足多场景多设备的需求。
...
- RS232在软件使用上基本和TTL一致，所以也是全双工通讯，在设备的两端都增加转换芯片(如MAX232)进行了电平的转换，即**电压+3~+15V表示逻辑0，电压-3~-15V表示逻辑1**，这样电平的差值可以很大概率防止翻转，增强了抗干扰能力，可以一定程度上增加传输距离，一般用于20m以内的远程传输，也是产品开发中常用的设计支持。
