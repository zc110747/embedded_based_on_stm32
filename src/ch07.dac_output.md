...
static BaseType_t drv_dac_init()
{
    ...
    // 4.config dac cannel
    sConfig.DAC_Trigger = DAC_TRIGGER_NONE;                 //定义触发源, 不需要触发
...
这里先讲解下声音输出的说明，扬声器是基于振动的原理，将电信号转换成声音信号的器件；在真实的环境下，由无数个点共同组成符合规则的一组连续可变电平信号，提供给扬声器，即可产生固定的声音。其中电平信号变化越密集，则还原的声音越真实。不过当实现这些电平信号时，单片机的性能有限，只能选取一部分点采样，按照一定的周期取样控制电平输出(这个取样就是音频的采样频率)，就可以实现预期的音频发声。

这里面有加个关键的特性，需要在设计上实现。

1. 音频的采样频率，常见有8k/16k/32k, 对应输出电压DAC的频率。
2. 音频的位数，常见有8bit/16bit, 对应输出电压DAC的位数。

这里以16k频率，16bit的一段wav音频，进行实现音频的输出；其实现步骤如下所示。

第一步就实现将wav音频转成DAC输出数组。
...
关于DAC位数，音频转换后是16bit的，而DAC位数是12bit的，直接使用肯定不行，那么要进行相应的转换才能被使用。这里有两种方法。

1. 使用DAC_ALIGN_12B_R配置，需要将音频原始数据右移4位，丢弃最低4位，输出即可；不过这需要在发送前对原始数据进行转换。
