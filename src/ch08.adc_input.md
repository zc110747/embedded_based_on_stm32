...
STM32F4的ADC模块为12位逐次逼近型模拟数字转换器，同时支持多达19个通道。ADC模块支持单次、连续、扫描和间断模式，同时结果可以左对齐或右对齐方式存储在数据寄存器中。ADC是采样电压的器件，不过配合外部硬件电路，如差分放大器，电阻分压等，可以用于采集电压、电流、位置、温度等信息。常见的应用中，电流可通过在接地端放置毫欧电阻，再通过差分放大器转换为电压测量；位置则可通过位移传感器，采集在全部行程占比获得；温度则可以采集NTC电阻电压值，计算NTC温敏电阻阻值，再通过公式反推计算的得出。可以说ADC模块是系统中比较重要的输入模块，一般用于系统控制(如运动中的电流，位置检测)、设备保护(过压/过流保护)等，发挥着至关重要的作用。
...
对于STM32F429来说，支持三路ADC，其中每个ADC都多达19个通道；其中ADCxIn0~ADCxIn15为外部ADC通道，ADCx为ADC模块，In0~In15为通道号，另外三个通道为芯片温度，基准电压和电池电压检测。对于每个通道对应的硬件I/O，需要参考STM32F429xx_Reference_Manual.pdf文档中关于引脚复用的说明。

再本例中，使用的为PA6作为模拟输入引脚，其支持ADC1_CHANNEL6和ADC2_CHANNEL6，这里选择ADC1_CHANNEL_6作为模拟部分的输入信号。

![image](image/08_02.png)

理解了这些，ADC的模块使用并不复杂。在硬件满足的情况下，触发启动ADC采样后，等待转换完成，即可从数据寄存器中读出ADC的采样值；通过一定公式转换即可获取实际的电压、电流或者温度值。对于ADC模块，具体的配置流程如下。
...
6. 以基准3.3V，12bit ADC为例，计算出实际电压值:vol = ADC_VAL/pow(2, 12)* Vref, 内部Vref默认为3.3V，可通过功能校准。
...
对于STM32的ADC模块，支持连续模式；可以扫描所有规则通道，写入数据寄存器，这时就需要有机制把数据读取出来。使用轮询机制检查EOC读取是一种方法，不过ADC的速度是很快的，这样占用的资源大，另外采用EOC中断进行数据读取也类似。
...
ADC模块是嵌入式系统中最重要得输入模块之一，通过采集ADC输入电压，可以获取电路上的电压、电流、温度、位置等信息；这些信息可用于系统监测、运动控制、过压过流保护、掉电保护等功能得实现。通过配置单片机，单次读取ADC数据十分简单，只需要配置好ADC模块，使能ADC模块，等待转换完成，即可读取转换后的数据。基于DMA+ADC连续读取的方式，采样数据虽然复杂些，但也并不是不能实现。在应用中，如何选择合适的窗口，以及何种方式处理采集到的数据信息，满足最终应用的需求，才是开发中的难点，也是和行业息息相关的知识点。
