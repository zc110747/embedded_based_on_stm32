...
timer编码器的使用方法主要是记录编码器返回A，B在指定时间里的计数，根据计数算出电机的转速。对于计数分为两部分组成。
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if (htim->Instance == TIM2)
    {
        ...
        if(encoder_data.encoder_num >= 0)
        {
            if(encoder_data.encoder_num  >=  encoder_data.last_encoder_num)
            {
                diff_encode_num = encoder_data.encoder_num - encoder_data.last_encoder_num; 
            }
            else
            {
                diff_encode_num = encoder_data.encoder_num + 65536*TIM_MAX_NUM - encoder_data.last_encoder_num;
            }
        }
        else
        {
