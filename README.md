...
对于芯片来说，输出的是I/O、PWM这些确定的信号，不过对于真实物理世界的控制，其背后是有一套完整的软硬件方案来实现的；

这里举几个例子，来说明实现具体应用的复杂性。

### 电机控制应用

PWM、GPIO是单片机应用中常见的输出接口，使用PWM驱动控制电机，也是十分常见的应用。

以最简单的有刷电机为例，从芯片的控制角度去理解，通过驱动芯片，使用如下引脚就能实现控制。

1. 使能
2. 方向
3. PWM输出速度控制

这也是典型的PWM控制方案，理论上负载相同的情况下，PWM对应着对应扭矩，也就对应着速度。但是现实不是，电机驱动着外部齿轮运转，会有负载的变化，控制就不是对于PWM值的要求，而是对可变负载下的速度要求。这就需要一整套反馈机制来处理。

这就需要建立编码器速度采集、电流环/速度环/位置环控制、PWM输出控制的综合应用。学习Timer定时器、PWM输出知识有用吗？肯定有的，但对于整个系统就肯定不是重点，甚至背后PID反馈链路也不困难，代码实现也不困难。但是根据硬件特性调整PID参数，适配当前硬件，就是十分复杂的应用了。

只能通过先建模计算出基础数据、再根据实际测试数据去微调，最终适配出稳定的参数。这还只是基础的有刷电机，如果复杂的BLDC、以及FOC应用，就需要掌握更多的知识。

### 无人机控制应用

上面的电机控制应用是实现了对单个电机的控制，但是对于具体的项目，如无人机开发，是需要四个电机控制的。

对于无人机来说，要实现前进、后退、上升、下降、悬停和转向功能，就需要根据多个电机来建模进行综合性控制。

其中采集部分包含IMU/GPS/气压计/磁力计等数据，进行状态估计，转换为每个电机的转速，转速再通过电机本身的控制链路反馈到真实硬件上。在这个流程中，每个动作都要适配硬件结构和物理特性，电机速度的增大和减少都需要通过准确的算法来处理，这些就是技术转换为真实应用的难点。
