...
static osThreadId_t LoggerTcpHandle, LoggerTxHandle;
...
void logger_tcp_task(void *argument)
{
...
GlobalType_t logger_client_init(void)
{
    ...
    LoggerTcpHandle = osThreadNew(logger_tcp_task, NULL, &TcpTask_attributes);
    if(LoggerTcpHandle == NULL)
    {
        return RT_FAIL;
    }
