...
{  
    ...
    while (1)
    {
        if(drv_tick_difference(tick, HAL_GetTick()) > 1000)
        {
            ...
            PRINT_LOG(LOG_INFO, HAL_GetTick(), "timer4 tick:%d", ger_timer_tick());
            
            PRINT_LOG(LOG_INFO, HAL_GetTick(), "cur_second:%d", get_cnt_per_second());
            
            LED_TOGGLE;
        }
        
        if(last_scan_key != tpad_scan_key())
        {
            last_scan_key = tpad_scan_key();
            
            PRINT_LOG(LOG_INFO, HAL_GetTick(), "key state change:%d", last_scan_key);
        }
    }
}
