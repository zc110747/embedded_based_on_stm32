...
📀 FreeRTOS工程项目：**code/03-01-1_STM32F429_FreeRTOS/**  
📀 STM32CubeMX生成FreeRTOS工程项目：**code/03-01-2_STM32F429_FreeRTOS_CubeMX/**

## 为什么使用RTOS

在了解RTOS前，先理解在单片机开发中，为什么要使用RTOS？事实上，在这篇之前的文章中，都是使用裸机开发来实现功能的，RTOS并不是一个必选项。使用主循环、中断通过状态机管理(类协程机制)的前后台系统，在时延要求的情况下使用定时器进行处理，采用DMA中断的方式进行数据收发，理论上裸机也能实现复杂的应用程序。

工作中场景要求如果达到us级别，往往RTOS软件也做不到准确时延，都是由硬件如定时器、DMA和中断等直接处理。看到这里，你可能越发觉得为什么用RTOS？其实相比裸机，RTOS在增加资源占用的情况下，大幅度的降低了开发难度。例如支持设备间通讯(CAN、USART)、外部器件读取(i2c、spi)、GUI刷新的设备；如果使用裸机，就需要维护复杂的状态检测、事件触发和通讯状态管理的机制，每一部分都要保证不发生堵塞，检查状态循环处理；来保证在主循环执行时不因为超时导致通讯超时、刷新帧率降低等问题。

使用RTOS后，就可以天然将上述功能拆分成多个任务来管理，降低了系统中各模块的耦合以及系统复杂度，除非时延要求特别严格，大部分情况下，分配到不同任务就可以实现目标，这正是RTOS的优势所在。在应用中，如果设备的资源允许，这里还是建议优先使用RTOS，这样对于后期的功能扩展和维护，会更加方便。这里讲解了为什么使用RTOS，后续将以FreeRTOS为例，开始进行说明。
...
🛠️ 将HAL库的时钟从Systick切换到普通Timer时钟，这是因为在FreeRTOS中，会使用Systick作为系统节拍，同时使用会有冲突(也可以FreeRTOS创建完成后，将HAL_IncTick()添加到FreeRTOS/portable/port.c中，不过这样每次重新生成项目时，HAL_IncTick()会丢失，不建议)  
🛠️ 在Middleware and Software Packs中添加FreeRTOS项目  
🛠️ 进行FreeRTOS项目配置，添加任务，并设置任务优先级，生成项目即可  
🛠️ 在生成项目中，添加需要处理的任务，此时可以实现FreeRTOS功能  

具体流程如下所示。

🧩 **切换时钟，在System Core/SYS选项下Timebase Source选择Timer**  

![image](./image/ch03_01_03.png)

🧩 **添加FreeRTOS项目，在Middleware and Software Packs中添加FreeRTOS项目**  

![image](./image/ch03_01_04.png)

🧩 **配置FreeRTOS项目，在FreeRTOS选项下，添加任务，并设置任务优先级，生成项目即可**  

![image](./image/ch03_01_05.png)
...
// 开启任务相关API函数
#define INCLUDE_vTaskPrioritySet             1      // 开启任务优先级设置函数
#define INCLUDE_uxTaskPriorityGet            1      // 开启任务优先级获取函数
#define INCLUDE_vTaskDelete                  1      // 开启任务删除函数
#define INCLUDE_vTaskCleanUpResources        0      // 开启任务资源清理函数
#define INCLUDE_vTaskSuspend                 1      // 开启任务挂起函数
#define INCLUDE_vTaskDelayUntil              1      // 开启任务延时直到指定时间函数
#define INCLUDE_vTaskDelay                   1      // 开启任务延时函数
#define INCLUDE_xTaskGetSchedulerState       1      // 开启任务调度器状态获取函数
#define INCLUDE_xTimerPendFunctionCall       1      // 开启定时器函数调用函数
#define INCLUDE_xQueueGetMutexHolder         1      // 开启队列互斥锁持有者获取函数
#define INCLUDE_uxTaskGetStackHighWaterMark  1      // 开启任务栈高水位标记获取函数
#define INCLUDE_xTaskGetCurrentTaskHandle    1      // 开启任务当前句柄获取函数
#define INCLUDE_eTaskGetState                1      // 开启任务状态获取函数
...
💡 对于访问资源的接口，包括中断接口和非中断接口，以末尾是否带ISR作为区分。中断接口表示在ISR中调用，非中断接口表示在非ISR中调用  
...
/// @brief 创建静态任务
/// @param pxTaskCode 静态任务函数指针
/// @param pcName 静态任务名称
/// @param ulStackDepth 静态任务栈大小
/// @param pvParameters 静态任务参数
/// @param uxPriority 静态任务优先级
/// @param puxStackBuffer 静态任务栈缓冲区
/// @param pxTaskBuffer 静态任务缓冲区
/// @return TaskHandle_t 静态任务句柄
...
notification接口用于实现对于任务通知的操作，是FreeRTOS的基础功能模块。对于FreeRTOS来说，notification是一种任务间通信的机制，用于实现任务之间的同步和协作，目前已经作为基础Task通讯的模块之一。对于notification模块的使能和控制，主要依赖如下两个宏定义。

💡 **configUSE_TASK_NOTIFICATIONS**，设置为1（或保留为未定义状态），此时都会开启notificaiton功能，设置为0，则关闭notification功能  
💡 **configTASK_NOTIFICATION_ARRAY_ENTRIES**，决定了每项任务的任务通知数组中的索引数，可根据实际需求设置，但不能够为0(默认值为1)  

当开启notify功能，会为每个任务创建一个任务通知数组，该数组大小为configTASK_NOTIFICATION_ARRAY_ENTRIES，每个数组元素用于存储任务的通知值，会占用一定内存；notify功能是默认开启的，如果确定不使用可以关闭，降低内存占用。
...
| xTaskNotifyStateClear | 清除任务当前通知状态 |
| ulTaskNotifyValueClear | 清除任务本身通知值 |

对于上述接口，比较核心的API如下所示。

```c
/// @brief 给任务发送通知，从索引0处发送通知
...
/// @brief 给任务发送通知并查询
/// @param xTaskToNotify 要发送通知的任务句柄
/// @param ulValue 要发送的通知值
/// @param eAction 通知操作类型
/// @param pxPreviousNotificationValue 通知前的值
/// @return BaseType_t 通知结果
...
/// @param xTicksToWait 等待时间，单位为系统节拍
/// @return 获取任务通知成功返回 pdPASS，否则返回 pdFAIL
...
/// @brief 获取任务通知(索引获取方式)
/// @param uxIndexToWaitOn 等待的索引
/// @param ulBitsToClearOnEntry 索引为uxIndexToWaitOn的任务通知值中要清除的位
/// @param ulBitsToClearOnExit 索引为uxIndexToWaitOn的任务通知值中要清除的位
/// @param pulNotificationValue 存储任务通知值的变量指针
/// @param xTicksToWait 等待时间，单位为系统节拍
/// @return 获取任务通知成功返回 pdPASS，否则返回 pdFAIL
...
对于notify接口，主要的知识点如下所示。

notify发送相关的接口会处理通知值，对于xTaskNotifyGive、xTaskNotifyGiveIndexed以及相应的中断内函数处理接口，目标的通知值获取时会自加1；而xTaskNotify和xTaskNotifyIndexed接口，则会根据eAction参数进行不同的处理，其中eAction支持的配置如下。

💡 eNoAction: 目标任务接收事件，但其通知值不会更新。在这种情况下， 不会使用ulValue(只通知事件，不修改值)  
💡 eSetBits: 目标任务的通知值将与ulValue    进行按位“或”操作  
💡 eIncrement， 目标任务的通知值将增加1，此时相当于调用xTaskNotifyGive  
💡 eSetValueWithOverwrite，目标任务的通知值将被设置为ulValue，覆盖之前的通知值  
💡 eSetValueWithoutOverwrite，目标任务的通知值将被设置为ulValue，如果目标任务当前没有挂起的通知，则其通知值将设置为 ulValue。如果目标任务已有挂起的通知，则其通知值不会更新，以免之前的值在使用前被覆盖  

另外对于任务通知的获取函数，如ulTaskNotifyTake、ulTaskNotifyTakeIndexed等，成功接到通知后，会根据上述描述的通知值eAction参数处理后进行范围。
...
// xNrHandle
void notify_reader_task(void *argument)
{
...
// xNwHandle
void notify_writer_task(void *argument)
{
    uint32_t notify_value = 0;
...
可以看到，直达任务通知可以在不额外创建的情况，通过多种接口实现类似信号量、消息、邮箱和事件组的功能，可以说从基础上简化了task间任务通讯的开发难度。
...
在本节中，对于FreeRTOS的应用场景、移植和配置方法、任务管理和线程同步方法、队列、流缓冲区、消息缓冲区、信号量、软件定时器、事件组等内容进行了详细的介绍。

对于FreeRTOS学习，还有另一个维度，就是了解其背后的工作和设计实现原理，不过一方面并不能快速运用的产品开发中，另一个也不是一篇文章就能讲解清楚，反而从应用角度出发，掌握这些知识已经足够在产品中使用，才是更有效的思路。

不同的场景下，还是需要有选择的使用相应接口实现功能。只触发单个事件，使用semaphore接口；中断中使用带ISR后缀的API；如果有固定长度的数据传输，使用queue接口；不定长数据格式包则使用message_buffer；多个事件组合则使用event_groups；短时互斥使用临界区，长时间资源互斥使用mutex。根据不同的场景需求有着相应的实践方案，熟练掌握并运用，才算真正的掌握FreeRTOS的功能应用。
