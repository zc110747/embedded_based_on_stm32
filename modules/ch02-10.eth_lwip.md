...
在单片机中实现网络服务是最复杂的应用之一，需要考虑到硬件驱动、协议栈的移植、应用层协议的实现等多个方面。现在基于STM32CubeMX已经很方便实现网络应用，但去理解网络的完整流程，解决开发中的软硬件问题，仍然是有挑战的项目。

一个基础的网络服务，例如实现Ping功能，也包含如下流程。

- ETH硬件驱动实现，包含I/O的配置，ETH功能配置(DMA和中断使能、MAC设置)等。
- PHY芯片的初始化和通讯接口配置，对于PHY来说包含两部分接口，用于配置的SMI接口和通讯的MII/RMII/RGMMII接口。通过SMI接口可以配置内部寄存器和获取PHY状态信息。
- 网络协议栈一般支持无RTOS和基于RTOS版本，考虑到网络应用的复杂度，一般情况下都是基于RTOS进行开发，需要进行RTOS的移植和兼容层的实现。
- 协议栈的移植，包含协议栈的配置、ip相关信息的配置，如IP地址、子网掩码、网关地址等，以及创建相应任务管理协议栈的收发功能。

完成上述步骤后，才能实现协议栈支持的基础功能，如ping、dhcp、socket通讯等功能。对于具体的网络应用，还需要在此基础上实现http、mqtt、telnet、snmp等应用层协议，远不是一篇文章能描述清楚的。这里也只能抛砖引玉，从应用的角度去整理和总结网络相关的知识，更深入的学习还需要配合其它书籍了解，本篇也参考以下书籍进行整理。
...
本节配合例程: **code/02-10_STM32F429_LWIP/**
...
物理层一般由国际组织进行定义，芯片和器件的生产厂商根据标准协议实现；常见硬件如ETH PHY、USB PHY、WIFI、蓝牙芯片等。对于开发者者可以不用去关心具体的实现细节，不过在器件选型、硬件设计和布线时需要考虑辐射发射(EMI)和电磁兼容性(EMC)；这部分在硬件初学者设计电路和布线经常被忽略，但往往是产品设计稳定可靠的关键，也是硬件工程师的最重要的能力之一。
...
- 网络接口层对应的就是上述的物理层；定义了物理接口、线缆、电平、数据格式、冲突处理等；包含设备驱动(PHY、MAC配置)，ARP和RARP等通讯媒介细节。
  - ARP协议：地址解析协议，用于根据IP地址查找MAC地址。
  - RARP协议：反向地址解析协议，用于根据MAC地址查找IP地址。
- 网络层可以理解为广义的协议层；网络层提供路由、寻址功能，保证对端的可达性，主要包含IP、ICMP、IGMP协议。
  - IP协议：网际协议，将数据尽可能快的从源节点送到目的节点(不可靠，无连接)。
  - ICMP协议: 如ping包就是基于该协议实现的，常用来做网路状况的测试。
  - IGMP协议：用于多播组成员的管理，如加入、退出、查询等。
- 传输层同样可以理解为广义的协议层，提供端对端的通讯连接，主要包含TCP、UDP协议。
  - TCP特点提供可靠的端对端通讯，通过三次握手建立连接，四次挥手断开连接。
  - UDP特点提供不可靠的端对端通讯，不保证数据的可靠到达，也不保证数据的顺序到达。
- 应用层则是在协议层基础上定义，实现的应用功能。开发者通过应用层来提供具体服务功能的实现，如http、ftp、telnet、mqtt、snmp等服务。
...
- 接收端获得数据开始一层层解包，最后由接收端指定层处理。因为组包和解包过程遵循着协议是完全透明的，那么相同的位置接收端和发送端的数据是一致的，就像实际物理连接起来一样，而为了区分协议不同的通讯功能，便有了分层的定义。
...
发送端处理就是将用户数据加上对应得数据包头格式并传递给下一层，最后通过网卡发送出去，这个过程称为封装；而接收端处理就是接收数据，去掉各层协议加上的报文首部，并检查报文首部的协议标识，从而确定接收的上层协议，这个过程称为解包。完整的TCP/IP数据格式如下所示。
...
- 外部硬件结构：包括网络接口、ETH PHY芯片和ETH MAC控制器组成。
- 内部驱动处理：配置ETH MAC驱动，实现模块初始化、数据接收和发送，以及数据处理。
- 协议栈移植: 移植RTOS和LWIP协议栈，适配协议栈功能。
- 应用实现：完成协议栈功能，完成上层应用，如socket客户端/socket服务端(tcp/udp)等。
...
MII/RMII接口用于数据通讯，其中包含发送和接收数据，以及时钟通讯接口和校验状态位。对于PHY芯片，驱动中通过SMI接口进行配置和查询。这里以LAN8720A为例，其关键信息包含PHY芯片的地址和关键寄存器参数。
...
- ETH驱动的移植，这部分参考官方例程中的ethernetif.c文件。
- 移植嵌入式实时系统RTOS，最初使用μC/OS-II，后来使用FreeRTOS。其实LWIP也支持非RTOS实现；不过这种情况在实际产品较少，这里不在赘述。
- 将LWIP协议添加到工程中，实现协议兼容层，这部分主要在sys_arch.c和sys_arch.h中实现。
- 完成协议栈的配置，主要在lwipopts.h中进行配置。
- 在主函数代码中添加协议栈初始化代码，设置ip，子网掩码，网关或启用dhcp功能，并初始化协议栈，创建相应任务。
- 实现上层应用，如socke客户端/socket服务端(tcp/udp)，http服务器等。
...
- PHY芯片初始化配置，内部的MAC层配置，以及数据的接收和发送功能。
- 外部PHY芯片初始化，主要包含接口初始化、PHY软复位和状态获取功能。
...
移植完成LWIP完成后，应用的实现就可以参考Linux端的socket应用。这里以TCP客户端为例，主要涉及的接口包含socket、connect、setsockopt、accept、bind、read、write等，其定义如下所示。
...
如此，tcp socket客户端的例程就实现了；主要实现功能为接收远端的数据，并打印到调试口上输出。当然在实际产品中，往往还要进一步解析接收数据、执行相应处理，最终实现网络相关的功能。
...
原因: 原PHY芯片停售，工厂直接使用另一个项目同封装的另一款PHY芯片替换，并未通知研发。导致读取内部寄存器状态不一致，导致初始化时未选择正确的工作模式和速度。
...
至此，关于STM32上的LWIP应用进行了说明。随着STM32CubeMX和其它芯片代码生成工具的发展，在单片机实现网络应用已经越来越简单。不过没有深入去理解PHY、MAC和LWIP、以及TCP/IP协议原理，当开发中如果遇到网络无法连接、通讯丢包或者应用层断开等问题，解决时再去理解由SDK直接生成的代码框架，其实难度反而增大了。
