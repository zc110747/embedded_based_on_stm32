...
对于上述信号中，RESET、LCD_BL、FMC_NWE、FMC_NOE这些信号就是字面意思，在之前的FMC_SDRAM也有说明，比较好理解。

有过FMC SDRAM的开发经验的，反而会发现有以下的差别。
...
可以看到，对于LCD来说并不需要SDRAM多达十几根的地址线，这是因为LCD控制器支持通过命令来选择行列地址和划定区域，写入时支持自增，那么关键点就需要实现如何控制命令/数据发送，以FMC_A18为例，只需要控制第A18引脚接口，其中1为内存、0为命令。

使用C语言结构体实现功能，则定义如下所示。
...
对于LCD的初始化，是需要读取ID，参考说明手册，实现一套指令完成对于驱动芯片的初始化。

在本例中，使用的屏幕驱动为NT35510。完整的LCD屏幕配置十分复杂，一般也都是使用厂商提供的驱动进行修改进行修改。
...
完成上述接口后，就可以实现最简单的刷屏应用了，不过这里就涉及颜色显示、字库处理的功能，下一步进行说明。

## 颜色和字库

对于颜色的理解涉及到自然界的知识，颜色是由不同波段的可见光组成的，不过对于屏幕，则选择相对独立的光源红(R)、绿(G)、蓝(B)三部分组成。

红绿蓝作为三原色，可以组合成我们大部分常见的颜色。对于各个颜色的深度，有通过字节位进行升级，常见的有8b、10、12bit等，对于单片机来说，常用的是RGB565/RGB888，这种都表示在屏幕上每个像素点中各种颜色占有的bit位数。

- RGB565：R(5bit)、G(6bit)、B(5bit)
- RGB888：R(8bit)、G(8bit)、B(8bit)

对于当前的LCD设备，就使用RGB565，正好2字节(uint16_t类型)，可以被上面的lcd_write_data写入。

这里就有简单的R，G，B数据合成RGB565，RGB888数据的公式，具体如下。

```c
// 将r，g，b数据转换为rgb565
static inline uint16_t convert_to_rgb565(uint8_t r, uint8_t g, uint8_t b)
{
    return (uint16_t)(((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3));
}
```

对于界面显示，只需要在页面的指定区域填充数据，即可正常的修改界面显示。

通过颜色的处理，我们就可以在界面上实现画点、画线、还有通过字库实现文字显示的工作，具体接口如下所示。
