...
在单片机中实现网络服务是最复杂的应用之一，需要考虑到硬件驱动、协议栈的移植、应用层协议的实现等多个方面。

现在基于STM32CubeMX已经很方便实现网络应用，但去理解网络的完整流程，解决开发中的软硬件问题，仍然是有挑战的项目。
...
📀 以太网应用例程：**code/02-10_STM32F429_LWIP/**

## 网络服务应用实现详解

一个基础的网络服务，例如实现Ping功能，主要包含如下流程。

🛠️ ETH硬件驱动实现，包含I/O的配置，ETH功能配置(DMA和中断使能、MAC设置)等  
🛠️ PHY芯片的初始化和通讯接口配置，对于PHY来说包含两部分接口，用于配置的SMI接口和通讯的MII/RMII/RGMMII接口。通过SMI接口可以配置内部寄存器和获取PHY状态信息  
🛠️ 网络协议栈一般支持无RTOS和基于RTOS版本，考虑到网络应用的复杂度，一般情况下都是基于RTOS进行开发，需要进行RTOS的移植和兼容层的实现  
🛠️ 协议栈的移植，包含协议栈的配置、ip相关信息的配置，如IP地址、子网掩码、网关地址等，以及创建相应任务管理协议栈的收发功能  

完成上述步骤后，才能实现协议栈支持的基础功能，如ping、dhcp、socket通讯等功能。

对于具体的网络应用，还需要在此基础上实现http、mqtt、telnet、snmp等应用层协议，远不是一篇文章能描述清楚的。这里也只能抛砖引玉，从应用的角度去整理和总结网络相关的知识，更深入的学习还需要配合其它书籍了解。
...
通讯协议的学习其实一直是嵌入式应用中的难点，在入门之初，虽然开发过涉及多种通讯接口类型的物联网设备，仍然不求甚解。在多年的学习总结后，才有了一套自己的理解；通讯协议可以理解为约定不同设备进行交互的一套方法的总和。虽然因为应用场景的不同，有不同的层级和功能，但最核心的都可以理解为如下结构。

🧩 **物理层**: 通讯之间物理硬件接口和连接方式

- **通讯连接方式**: 有线连接或无线射频连接
- **通讯物理接口**：RJ45、USB-TYPEC、RS485、RS232、SPI/I2C、CAN等
- **通讯方式**: 有线主要是工作电平(高低电平、差分信号等)，无线主要是工作频段(2.4G、5G、WIFI等)，对于复杂信号往往还支持频分和时分数据处理功能
- **信号编解码方式**: 如曼彻斯特编码、差分曼彻斯特编码、NRZ编码、RZ编码等

🧩 **协议层**: 确定数据的通讯格式

- **帧格式**: 如帧头、数据长度、协议版本信息、数据包格式、校验码和帧尾等；约定双方通过相同的协议进行加解码功能。
- **数据格式**: 如ASCII、UTF-8、UTF-16、UTF-32、HEX、BASE64、BINARY等。

🧩 **应用层**: 用于定义具体功能需求的应用功能

- **功能需求**: 如http、mqtt、telnet、snmp等功能。

物理层一般由国际组织进行定义，芯片和器件的生产厂商根据标准协议实现；常见硬件如ETH PHY、USB PHY、WIFI、蓝牙芯片等。

对于软件开发者可以不用去关心具体的实现细节，不过在器件选型、硬件设计和布线时需要考虑辐射发射(EMI)和电磁兼容性(EMC)；这部分在硬件初学者设计电路和布线经常被忽略，但往往是产品设计稳定可靠的关键，也是硬件工程师的最重要的能力之一。

协议层可以将两个设备建立逻辑上的连接，从而在多设备互联的情况下实现一对一或多对多的通讯。对于复杂的通讯协议，一般会进一步分解成多层协议格式来应对不同的应用场景。协议层往往只会定义大体的框架结构，由设计者根据产品的需求进行填充来实现具体的应用，也是嵌入式软件中最常涉及的部分。

在正常的分层概念中，其中物理层代表实际的硬件接口以及相应的驱动和接口协议，协议层是组包和解包的软件处理逻辑，应用层则定义具体的应用功能。通讯的双方都包含这三部分功能，最终完成数据在多个设备之间的流转。基于此思路对TCP/IP进行理解，就有着如下的认知。

🧩 **网络接口层**，对应的就是上述的物理层；定义了物理接口、线缆、电平、数据格式、冲突处理等；包含设备驱动(PHY、MAC配置)，ARP和RARP等通讯媒介细节。

- **ARP协议**：地址解析协议，用于根据IP地址查找MAC地址。
- **RARP协议**：反向地址解析协议，用于根据MAC地址查找IP地址。

🧩 **网络层**，属于广义的协议层；网络层提供路由、寻址功能，保证对端的可达性，主要包含IP、ICMP、IGMP协议。

- **IP协议**：网际协议，将数据尽可能快的从源节点送到目的节点(不可靠，无连接)。
- **ICMP协议**: 如ping包就是基于该协议实现的，常用来做网路状况的测试。
- **IGMP协议**：用于多播组成员的管理，如加入、退出、查询等。

🧩 **传输层**，同样属于广义的协议层，提供端对端的通讯连接，主要包含TCP、UDP协议。

- **TCP协议**，特点提供可靠的端对端通讯，通过三次握手建立连接，四次挥手断开连接。
- **UDP协议**，特点提供不可靠的端对端通讯，不保证数据的可靠到达，也不保证数据的顺序到达。

🧩 **应用层**，在协议层基础上定义，实现的应用功能。

开发者通过应用层来提供具体服务功能的实现，如http、ftp、telnet、mqtt、snmp等服务。
...
🧩 **网络请求**，主要是客户端发起，服务端接收，具体流程如下所示。

![image](./image/ch02_10_01.png)

🧩 **网络应答**，服务端根据请求返回给客户端数据，具体流程如下所示。

![image](./image/ch02_10_02.png)

网络请求和应答组成了一次完整的通讯过程，其流程和处理是镜像的；按照相同的格式封装和解包数据，这就能够保证在物理空间上不同设备之间的通讯能够正常进行。具体流程如下所示。

💡 发送端将待发送数据经过一层层组包，最后调用驱动程序，通过网卡发送；复杂情况下要经过多个路由器，最终到达接收端  
💡 接收端获得数据开始一层层解包，最后由接收端指定层处理。

组包和解包过程遵循着协议是完全透明的，那么相同的位置接收端和发送端的数据是一致的，就像实际物理连接起来一样，而为了区分协议不同的通讯功能，便有了分层的定义  

整个流程中的代码处理大致如下所示。

```c
// 发送端发送数据流程
app_translate_data -> tcp_process() -> ip_process() -> eth_translate() -> tdata......

// 接收端接收数据流程
app_receive_data <- tcp_process() <- ip_process() <- eth_receive()  <- rdata......
```

发送端处理就是将用户数据加上对应得数据包头格式并传递给下一层，最后通过网卡发送出去，这个过程称为封装；而接收端处理就是接收数据，去掉各层协议加上的报文首部，并检查报文首部的协议标识，从而确定接收的上层协议，这个过程称为解包。

完整的TCP/IP数据格式如下所示。

![image](./image/ch02_10_03.png)

可以看到，通过TCP/IP协议的一层层的封装和解包，数据在发送端和接收端之间进行传递。

对于TCP/IP设备来说，其发送端和接收端根据类型的不同，包含C/S类型和B/S类型。

💡 **C/S类型**：传统的客户端软件/服务器模式，客户端和服务器通过UDP/TCP协议进行通讯。
💡 **B/S类型**：浏览器/服务器模式，建立在TCP/IP协议基础上，通过浏览器支持的HTTP/HTTPS协议进行通讯，实现web应用。

上述资料描述了网络相关的协议栈，以及数据报文的解析，大致对TCP/IP网络也就有了一定的认识。下面以STM32开发板为例实践网络相关的功能。

## STM32硬件接口和驱动

对于STM32中使用TCP/IP通讯，其中分为外部硬件结构、内部驱动处理、协议栈移植和应用实现四个部分组成，具体如下所示。

🛠️ 外部硬件结构：包括网络接口、ETH PHY芯片和ETH MAC控制器组成  
🛠️ 内部驱动处理：配置ETH MAC驱动，实现模块初始化、数据接收和发送，以及数据处理  
🛠️ 协议栈移植: 移植RTOS和LWIP协议栈，适配协议栈功能  
🛠️ 应用实现：完成协议栈功能，完成上层应用，如socket客户端/socket服务端(tcp/udp)等  
...
| RXP | 网口接收数据引脚+ |
| RXN | 网口接收数据引脚- |
| LED_Y(黄灯) | 网口连接状态，连接成功点亮 |
| LED_G(绿灯) | 网口通讯状态，通讯时闪烁 |

连接引脚的就是我们常说的PHY芯片，PHY芯片的功能主要完成模数转换。发送数据时，将MCU的数字信号转变为模拟信号进行发送；接收数据时，将接收的模拟信号转变为数字信号进行处理。PHY芯片支持网速和工作方式的软件配置或自动协商，是网络活动中比较重要的部分。对于大部分MCU芯片，PHY都未集成到芯片中，需要外置PHY芯片配合使用。

对于本例程，使用的PHY芯片为LAN8720A，其硬件连接如下所示。

![image](./image/ch02_10_05.png)
...
MII/RMII接口用于数据通讯，其中包含发送和接收数据，以及时钟通讯接口和校验状态位。

对于PHY芯片，驱动中通过SMI接口进行配置和查询。这里以LAN8720A为例，其关键信息包含PHY芯片的地址和关键寄存器参数。

手册可参考链接: <https://atta.szlcsc.com/upload/public/pdf/source/20151012/1457707444106.pdf>

对于PHY芯片来说，以下特征比较重要。

🧩 **PHY芯片的地址**，在多PHY情况下，需要根据这个地址区分配置的PHY芯片。因为本设备只有一个网口，不需要考虑多PHY情况，设定地址为0即可。

![image](./image/ch02_10_06.png)

🧩 **设置PHY芯片工作方式**，主要包含网络速率：100M/10M，双工情况：全双工/半双工，是否自动协商等，如果设置自动协商在连接时要复位下PHY芯片，保证能够正确获得配置信息。

![image](./image/ch02_10_07.png)

🧩 **获取PHY当前的芯片状态**，包含连接状态、网络速率、工作模式、是否自动协商等等

![image](./image/ch02_10_08.png)

对于大部分PHY芯片，理论上都需要包含此功能，不过寄存器的位置和功能可能都不一致，这就需要根据PHY芯片的手册去适配，也是物理层实现的主要工作。

## 基于STM32CubeMX构建LWIP应用

在最初接触单片机网络开发时，STM32CubeMX还不存在，只能自己参考LWIP协议的方案去移植，具体步骤如下所示。

🛠️ ETH驱动的移植，这部分参考官方例程中的ethernetif.c文件  
🛠️ 移植嵌入式实时系统RTOS，最初使用μC/OS-II，后来使用FreeRTOS。其实LWIP也支持非RTOS实现；不过这种情况在实际产品较少，这里不在赘述  
🛠️ 将LWIP协议添加到工程中，实现协议兼容层，这部分主要在sys_arch.c和sys_arch.h中实现  
🛠️ 完成协议栈的配置，主要在lwipopts.h中进行配置  
🛠️ 在主函数代码中添加协议栈初始化代码，设置ip，子网掩码，网关或启用dhcp功能，并初始化协议栈，创建相应任务  
🛠️ 实现上层应用，如socke客户端/socket服务端(tcp/udp)，http服务器等  

记得我第一次从无到有调通LWIP整个流程，并运行稳定，花了大概有一个多月的时间。后续在这基础上实现http/telnet/snmp的完整应用，实现了远程管理，又大概花费了接近三个月时间，才差不多完善整个项目。如今使用英飞凌的DAVE、以及ST的STM32CubeMX工具，通过生成代码的方式实现，大概只花了1~2天的时间，就完整实现了LWIP的移植和调试的工作；STM32CubeMX可以说大幅度降低了开发难度。

这里以STM32F429芯片为例，描述如何通过STM32CubeMX来实现LWIP应用。

### 工程创建

🧩 **选择STM32F429IGT6，配置时钟晶振，工作频率，调试接口等**

![image](./image/ch02_10_09.png)

🧩 **配置FreeRTOS，其中SysTick作为系统时钟，HAL库的时钟源切换到TIM1**

![image](./image/ch02_10_10.png)

🧩 **配置ETH模块，修改接口为RMII模块，接收长度为1536，中断接收模式，并根据硬件原理图调整GPIO**

通过上节可以知道连接使用的是RMII模式，PHY芯片对应LAN8720芯片，进行相应的配置。

![image](./image/ch02_10_11.png)

🧩 **添加LWIP协议，选择底层的rtos，并配置协议栈，并选择对应的phy芯片**

![image](./image/ch02_10_12.png)
...
![image](./image/ch02_10_13.png)

通过ping命令测试网络是否正常，具体如下所示。

![image](./image/ch02_10_14.png)

## LWIP代码关键代码分析

这里说下对于LWIP移植比较关键的代码文件。

💡 **Application/User/Core/main.c**：主循环入口代码，进行驱动初始化、rtos使能和任务创建  
💡 **Application/User/LWIP/Target/ethernetif.c**：PHY物理层驱动代码，phy和mac的配置，网络数据的物理层接收和发送接口  
💡 **Drivers/BSP/Components/lan8742.c：PHY芯片代码**，适配PHY驱动地址和通讯交互，控制PHY工作模式，以及PHY的配置参数  
💡 **FreeRTOSConfig.h**：freertos管理的配置文件，用于支持rtos的相关资源  
💡 **lwipopts.h**：lwip管理的配置文件，用于控制lwip的资源  

### 物理层配置

对于LWIP协议支持STM32F429开发板来说，物理层主要包含以下配置。

🛠️ PHY芯片初始化配置，内部的MAC层配置，以及数据的接收和发送功能  
🛠️ 外部PHY芯片初始化，主要包含接口初始化、PHY软复位和状态获取功能 
...
对于物理层另一个就是配置MAC功能，主要是配置MAC的工作模式、帧检测、硬件检查等功能；选择合适的配置可以提高发送和接收效率，避免错误帧丢失。
...
下面开始说明LWIP协议中的功能。对于LWIP来说，其移植主要分为以下部分，具体如下所示。

🛠️ LWIP模块的初始化  
🛠️ 系统兼容层的实现  
🛠️ LWIP协议栈的配置  

其中LWIP的初始化比较简单，主要实现位于lwip.c文件中，按照顺序调用如下接口即可，具体如下所示。
...
这一节主要说明lwipopts.h中的主要配置项，这里提供曾经在应用中调整的比较完善的配置。
...
移植完成LWIP完成后，应用的实现就可以参考Linux端的socket应用。

这里以TCP客户端为例，主要涉及的接口包含socket、connect、setsockopt、accept、bind、read、write等，其定义如下所示。
...
如此，tcp socket客户端的例程就实现了；主要实现功能为接收远端的数据，并打印到调试口上输出。

当然在实际产品中，往往还要进一步解析接收数据、执行相应处理，最终实现网络相关的功能。

## LWIP网络开发问题和解决办法

这里分享曾经使用ETH LWIP产品开发和量产时遇到的问题和解决办法。
...
解决办法: 硬件上强制把MCU的MII_ER脚拉低、连接PHY对应引脚上，或者软件配置不丢弃错误帧；后续通过硬件升版本正确连接引脚到MCU对应引脚上，修复解决。

### issue-02

**PHY芯片被生产更换为另一款同样封装的芯片，出厂测试通过网络访问设备卡顿**。

原因: 原PHY芯片停售，工厂直接使用另一个项目同封装的另一款PHY芯片替换，并未进行通知，导致初始化时未选择正确的工作模式和速度。

解决办法: 驱动里增加读PHY芯片id代码，并执行相应初始化流程，兼容了这款PHY芯片。

### issue-03

**网线中继使用普通导线，且和电源线并排，当功率大时发生断网**。

原因: 硬件和结构设计，有一段线缆需要经过狭窄通道；为了通过方便，电子设计两端用了网线，中间通道部分直接走的普通线缆作为网线中继。现象为实验室测试时通讯正常，量产机测试电机堵撞和EMC测试中，应用层通讯有概率会连接超时。应用层有重发机制以及超时处理，一段时间不正常通讯，才会认为断开。所以属于概率性，大概1%左右。

解决办法：刚开始被归为应用层问题，后来量产机通过"ping命令+电机堵撞"场景模拟，发现ping断开，网口也会短时间停止闪烁，确定是硬件问题。进一步分析确定为网线中继问题，后修改为全程都使用网线解决。之前测试未发现原因是测试机和量产机器连线不同，测试机因为线缆未完成设计，通过外接网线直连，量产机才使用内部线缆的中继线方案。

### issue-04

**网络上IP层数据包过多，导致访问LWIP卡顿**。

原因: 在FPGA管理的环网链路中，使用单片机实现服务器应用，同时单片机网络也连接在环网回路中；单片机同样会收到大量的环网内数据包，来不及处理，导致底层堵塞丢弃大量正常的数据包，造成卡顿。

解决办法: lwip的底层数据接收流程eth interrupt => ethernetif_input => tcpip_thread，在这一步调用对应的接收函数进行处理，不符合要求的数据包才会被丢弃。这里解决办法是将丢弃提前，在ethernetif_input中过滤ip目的地址非广播包以及自己ip地址的数据报文，符合要求的才会被传递到tcpip_thread中处理，解决了堵塞问题。

## 总结说明

至此，关于STM32上的LWIP应用进行了说明。随着STM32CubeMX等代码生成工具的发展，在单片机实现网络应用已经越来越简单。不过没有深入去理解PHY、MAC和LWIP、以及TCP/IP协议原理，当开发中如果遇到网络无法连接、通讯丢包或者应用层断开等问题，解决时再去理解由SDK直接生成的代码框架，其实难度反而增大了。

这并不是说不该用SDK，既然能大幅度提高效率，使用SDK反而是最优选择。不过在学习单片机网络应用过程中，还是要花时间去理解执行的流程，不能生成完代码，ping通了就认为学习掌握了网络知识，这是远远不足的。如果时间充足，还是单步调试下，配合代码理解LWIP的执行流程，另外LWIP的代码也是十分经典的C语言开源代码，包含很多C语言协议解析技巧，十分值得去了解学习。
