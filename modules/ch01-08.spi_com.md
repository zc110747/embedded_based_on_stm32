...
SPI是4-wire全双工的通讯总线，注意用于芯片和芯片，芯片和器件之间的数据传输通讯。
...
📀 SPI模块例程：**code/01-08_STM32F429_SPI/**

## SPI模式和接口

对于STM32的SPI接口，支持模式和接口如下所示。
...
SPI设计上为主从结构，在工作时通过主机提供时钟触发信号，发送数据的同时从接收引脚上读取数据，同时实现数据接收和发送功能，因此为全双工模式。不过对于STM32来说，也支持3-wire半双工模式，即只发送或者只接收，此时数据脚可以分时进行发送和接收，通过`__HAL_SPI_1LINE_TX`和`__HAL_SPI_1LINE_RX`进行控制数据发送和接收，用于适配部分特殊的外部器件。

SPI设计上没有类似I2C的地址来区分从机设备，对于多从机的通讯，需要依赖CS引脚来选中芯片工作。当SPI总线上有多个芯片使能，写入不影响，但多从机读取时会发生总线竞争，有可能导致接收数据异常。

⚠️注意：**使用单个SPI管理多个外部器件时，设计上需要每个芯片拥有独立的片选控制CS引脚**。

## SPI系统框架说明

SPI的框架如下所示。

![image](./image/ch01_08_01.png)
...
💡 NSS功能用于控制CS片选引脚，设置为Soft可以作为普通引脚控制，也可以通过SSI位控制，硬件模式则作为主SPI时，在发送数据时会强制拉低NSS引脚，从而实现自动片选控制  
💡 其它的是与数据通讯相关的参数配置，包含波特率、发送先后顺序(MSB或者LSB)、是否带CRC校验等，这些都是与具体器件有关，需要根据实际情况进行配置  
...
BaseType_t drv_spi_init(void)
{
    ...
    hspi5.Init.Direction = SPI_DIRECTION_2LINES;    // 双线模式，发送/接收独立线缆
    hspi5.Init.DataSize = SPI_DATASIZE_8BIT;        // 8bit模式，指出8/16bit模式
    ...
    hspi5.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;  // 禁止硬件CRC校验功能(独立的crc校验模块，和数据位长度有关，8bit表示crc8，16bit表示crc16)
...
![image](./image/ch01_08_03.png)

这个主要涉及硬件的连接，对于软件部分，我们更关心如何去读取ADXL345的加速度数据。这个就要去看手册中的读取波形说明，详细图形如下所示。

![image](./image/ch01_08_04.png)

从上图可以看到，数据的格式如下"地址功能区 + 数据区"，其中地址功能区功能如下。

💡 R(1)/W(0)位，表示读写，其中1表示读，0表示写  
💡 MB位，用于指定多字节读取，其中0表示单字节，1表示多字节  
💡 [A5:A0]位，用于指定寄存器地址  

其中MB的信息来自于文档说明如下。

![image](./image/ch01_08_05.png)

另外重要的信息，就是内部寄存器的地址，这里最重要的就是0x32~0x37这六个寄存器，表示三轴数据信息。

![image](./image/ch01_08_06.png)
...
{
    // 多字节读取
    // 0x32为寄存器地址，bit6为1表示多字节读取，bit7为1表示读功能 => 0xF2
    HAL_StatusTypeDef status;
    uint8_t rx_buffer[7];
    uint8_t tx_buffer[7] = {0xF2, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

    ADXL345_CS_LOW;
    status = HAL_SPI_TransmitReceive(&hspi5, tx_buffer, rx_buffer, 7, 20);        // 发送寄存器地址和读写功能位，多字节处理位
    ADXL345_CS_HIGH;
...
💡 硬件接口和内部框图，一个涉及PCB设计，另一个在软件开发中可以分析问题  
💡 通讯时序图，软件实现需要满足此时序才能正确工作，在开发时需要判断软件能否时序时序  
💡 寄存器和特殊功能位，寄存器影响芯片配置和数据读取，特殊功能则可能是芯片特有功能，如果通讯失败，可以判断是否是未设置相应特殊功能位  

## SPI软件模拟实现

目前大部嵌入式芯片都支持2~3路SPI接口，但实际应用中因为硬件电路限制，往往可能无法将SPI器件连接到硬件的SPI接口，这就需要由I/O来模拟SPI的实现。

这里描述下CPOL、CPHA为0的SPI实现方式，具体流程如下所示。

🛠️ 将SPI时钟下拉，从高位到低位设置MOSI的值  
🛠️ 拉高SPI的时钟，同时从第一个边沿后读取数据  
🛠️ 循环执行8次，写入8bit数据，同时读取8bit数据  
🛠️ 将SPI的时钟拉低，结束一个Byte的数据读取  
