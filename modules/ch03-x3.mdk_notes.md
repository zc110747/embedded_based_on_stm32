...
MDK编译仿真调试时，代码停止在BKPT指令，一般发生在同时使用stdio.h标准库内容时。

原因： stdio.h中的printf函数在标准库时默认使用了__use_no_semihosting，如果不存在则会在仿真时停止在BKPT指令。

解决办法一： 勾选microlib，该选项会在编译时链接microlib库，该库中没有__use_no_semihosting函数，同时实现fputc函数。
解决办法二： 添加__use_no_semihosting相关导入信息，该函数在编译时链接，同时实现fputc函数。
...
{  
    return uart_put_char((uint8_t)ch);
}
```
...
直接开始下一小节: [基于vscode的arm-none-eabi-gcc开发和调试](./ch03-x4.vscode_gcc_openocd.md)
